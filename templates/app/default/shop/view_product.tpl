		...
		<div class="grid auto-rows-min grid-cols-12 gap-4 mb-4">
			<div class="col-span-4">
				...
				<div class="grid grid-cols-6 gap-2">
					<img src="{$product.picture}" alt="{$product.title}" class="rounded-lg col-span-6">
					{if count($images) eq 1}
						{assign var="colspan" value="col-span-6"}
					{elseif count($images) eq 2}
						{assign var="colspan" value="col-span-3"}
					{else}
						{assign var="colspan" value="col-span-2"}
					{/if}
					{foreach from=$images key=key item=image}
						<img src="{$image.filename}" alt="{$product.title}" class="rounded-lg {$colspan}">
					{/foreach}
				</div>
			</div>
			<div class="col-span-8">
				...
				<div class="px-4 grid grid-cols-3 gap-4">
					<div class="col-span-2">
						<h1 class="text-2xl font-bold text-[#34445C]">{$product.title}</h1>
						...
						<div class="border-r border-gray-200 pb-16 pr-8 pt-6">
							...
							<div>
								...
								<div class="space-y-6">
									<p class="text-sm text-gray-900">{$product.annotation}</p>
								</div>
							</div>
								...
								<div class="mt-2">
									<ul role="list" class="list-disc space-y-1 pl-4 text-sm">
										{foreach from=$options key=key item=item}
											<li class="text-gray-400">{$item.title}: <span class="text-gray-600">{$item.value}</span></li>
										{/foreach}
									</ul>
								</div>
							</div>
					...
					<div class="col-span-1">
						...
						<div class="mt-4">
							...
							<p class="text-3xl tracking-tight text-gray-900">{$product.price} руб.</p>
									...
									<fieldset class="mt-4">
										<div class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
											{foreach from=$variants key=key item=variant}
												<label class="group relative flex items-center justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none cursor-pointer bg-white text-gray-900 shadow-sm {if $variant@iteration is div by 4}border-indigo-500{/if}">
													<input type="radio" name="size-choice" value="S" class="sr-only" aria-labelledby="size-choice-2-label">
													<span id="size-choice-2-label border">{$variant.name}</span>
													<!--
