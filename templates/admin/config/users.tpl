        ...
        <div class="bg-white shadow rounded-md h-full w-3/4 p-4 relative overflow-hidden">
            ...
            <div class="inline-flex rounded shadow-sm mb-4 float-right" role="group">
                {foreach from=$user_roles key=key item=item}
                    <a href="{url params=['role_id'=>$item.id_item, keyword=>null]}" class="{if $item@first}rounded-l-lg border{/if}{if $item@last}border{/if}{if !$item@last && !$item@first}border-b border-t{/if}  py-2 px-4 text-sm font-medium text-gray-900 bg-transparent border-gray-300 hover:bg-gray-100 hover:text-gray-900 focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-300 focus:text-gray-900">
                        {$item.name}
                    </a>
                {/foreach}
                ...
                {if isset($role_id)}<input value="{$role_id}" name="role_id" type="hidden">{/if}
                ...
                <a href="{url params=['role_id'=>null, keyword=>null]}" class="border rounded-r-md py-2 px-4 text-sm font-medium text-gray-900 bg-transparent border-gray-300 hover:bg-gray-100 hover:text-gray-900 focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-300 focus:text-gray-900">
                    Сбросить
                </a>
            </div>
            </form>
                ...
                {foreach from=$users key=key item=item}
                    <tr class="bg-white border-b border-dashed">
                        <td class="p-3 text-sm font-medium whitespace-nowrap">
                            <a href="/admin/?mod=pages&act=page&id={$item.id_item}" class="text-blue-500">{$item.name}</a>
                        </td>
...
{*                            <a href="/users/{$item.id_item}" class="inline-flex items-center px-2 py-2 bg-green-500 hover:bg-green-500 text-white rounded-md">*}
{*                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">*}
...
{*                            </a>*}
                            ...
                            <a href="/admin/?mod=config&act=user&id={$item.id_item}" class="inline-flex items-center px-2 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                </svg>
                            </a>

                            <a href="#" class="delete inline-flex items-center px-2 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md" data-id="{$item.id_item}" data-action="delete" data-url="/admin/?mod=config&act=users" data-title="{$item.name}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
