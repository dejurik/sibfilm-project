...
<form method="post" enctype="multipart/form-data" id="product_form">
                ...
                <div class="">
                    <div class="flex flex-wrap justify-between">
                        <div class="items-center ">
                            ...
                            <span class="text-gray-500 text-xs font-semibold mr-2 py-0.5">{if isset($view.id_item)}Редактировать {else}Добавить {/if}товар</span>
                        </div>
    ...
    {if isset($status)}
    <div class="flex p-4 mb-4 {if $status.error eq true}bg-red-100 border-red-700{else}bg-green-100 border-green-700{/if} border-t-4" role="alert">
    ...
    <div class="flex justify-between items-stretch mb-12">
                            ...
                            {function name=category_select level=0}
                                {foreach $categories as $item}
                                    <option value='{$item->id_item}'
                                            {if isset($view.category_id) && $view.category_id eq $item->id_item}selected="selected"{/if}
                ...
                <div class="mt-5">
                    ...
                    <div class="mt-2 relative">
                        <select name="type_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            <option {if isset($view.type_id) && $view.type_id eq 'product'}selected="selected"{/if} value='product'>Товар</option>
                            <option {if isset($view.type_id) && $view.type_id eq 'video'}selected="selected"{/if} value='video'>Видеофильм</option>
                            <option {if isset($view.type_id) && $view.type_id eq 'audio'}selected="selected"{/if} value='audio'>Аудиоальбом</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                        </div>
                ...
                <div class="mt-5 grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div class="col-span-2">
                        <div class="mb-0">
                            ...
                            <div class="mt-2">
                                <input value="{if isset($view.price)}{$view.price}{/if}" name="price" type="text" placeholder="Цена" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-2 px-4 leading-tight
                    ...
                    <div class="col-span-2">
                        <div class="mb-0">
                            ...
                            <div class="mt-2">
                                <input value="{if isset($view.old_price)}{$view.old_price}{/if}" name="old_price" type="text" placeholder="Старая цена" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-2 px-4 leading-tight
					focus:outline-none focus:bg-white focus:border-gray-500">
                            </div>
                        </div>
                    </div>
        ...
        <div class="w-6/12 bg-white shadow rounded p-4 mr-2">
            <div class="w-full mb-5">
                ...
                {foreach from=$variants key=key item=variant}
                    <input name="current_variants[{$variant.id_item}]" type="hidden" value="{$variant.id_item}" />
                {/foreach}

                <div class="dd" id="sort_variant">
                    <ol class="dd-list" id="variants_list">
                        {foreach from=$variants key=key item=variant}
                        <li class="dd-item dd3-item mb-1" data-id="{$variant.id_item}">
                            ...
                            <div class="flex flex-wrap content-start bg-slate-100 pl-16 pr-2.5 py-2.5 min-h-[54px] rounded border bg-slate-100">
                                <div class="flex items-center h-8 w-2/6">
                                    <input name="variants[id_item][]" type="hidden" value="{$variant.id_item}" />
                                    <input name="variants[name][]" value="{$variant.name}" type="text" placeholder="Название варианта"
                                ...
                                <div class="ml-2 flex items-center h-8 w-1/6">
                                    <input name="variants[duration][]" value="{$variant.duration}" type="text" placeholder="Длительность"
                                           class="text-xs py-2 block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded px-2 focus:outline-none focus:bg-white focus:border-gray-500">
                                </div>

                                <div class="ml-2 flex items-center h-8 w-1/6">
                                    <input name="variants[author][]" value="{$variant.author}" type="text" placeholder="Автор"
                                           class="text-xs py-2 block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded px-2 focus:outline-none focus:bg-white focus:border-gray-500">
                                </div>
                                <div class="ml-auto self-center h-8">
                                    <button class="attach_images bg-slate-200 hover:bg-slate-700 text-slate-500 hover:text-white w-8 h-8 self-center rounded" type="button" data-variant="{$variant.id_item}">
                                    ...
                                    <button class="attach_files bg-slate-200 hover:bg-slate-700 text-slate-500 hover:text-white w-8 h-8 self-center rounded" type="button" data-variant="{$variant.id_item}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mx-1.5 my-1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m18.375 12.739-7.693 7.693a4.5 4.5 0 0 1-6.364-6.364l10.94-10.94A3 3 0 1 1 19.5 7.372L8.552 18.32m.009-.01-.01.01m5.699-9.941-7.81 7.81a1.5 1.5 0 0 0 2.112 2.13" />
                                        </svg>
                                    </button>
                                    <button class="delete_variant bg-slate-200 hover:bg-slate-700 text-slate-500 hover:text-white w-8 h-8 self-center rounded" type="button" data-variant="{$variant.id_item}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mx-1.5 my-1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                        </svg>
                                    </button>
                                </div>
                                <div class="grid gap-2 mt-2 grid-cols-7 w-full hidden" id="images-container-{$variant.id_item}">
                                   {foreach from=$images key=key item=image}
                                   {$variant_images_id=$variant.images|json_decode nocache}
                                    <div class="flex flex-col items-center p-2 rounded border {if is_array($variant_images_id)}{if in_array($image.id_item, $variant_images_id)}border-blue-400 bg-blue-100{else}bg-slate-100{/if}{/if}">
                                        <input type="checkbox" id="image-list-{$image.id_item}-{$variant.id_item}" name="variants[images][{$variant.id_item}][]" value="{$image.id_item}" {if is_array($variant_images_id)}{if in_array($image.id_item, $variant_images_id)}checked="checked"{/if}{/if} class="hidden"/>
                                        <label for="image-list-{$image.id_item}-{$variant.id_item}">
                                            <img class="object-cover w-full rounded aspect-square" src="{$image.filename}" alt="">
                                        </label>
                                    </div>
                                    {/foreach}
                                </div>
                                <div class="grid gap-2 mt-2 grid-cols-1 w-full hidden" id="files-container-{$variant.id_item}">
                                    {foreach from=$files key=key item=file}
                                        {$variant_files_id=$variant.files|json_decode nocache}
                                        <div class="flex flex-col items-center p-2 rounded border {if is_array($variant_files_id)}{if in_array($file.id_item, $variant_files_id)}border-blue-400 bg-blue-100{else}bg-slate-100{/if}{/if}">
                                            <input type="checkbox" id="file-list-{$file.id_item}-{$variant.id_item}" name="variants[files][{$variant.id_item}][]" value="{$file.id_item}" {if is_array($variant_files_id)}{if in_array($file.id_item, $variant_files_id)}checked="checked"{/if}{/if} class="hidden"/>
                                            <label for="file-list-{$file.id_item}-{$variant.id_item}">
                                                <div class="flex items-center h-6">
                                                    ...
                                                    <span class="font-medium text-slate-500 text-xs">{if empty($file.description)}{$file.filename|substr:20}{else}{$file.description}{/if}</span>
                                                </div>
                                            </label>
                                        </div>
                                    {/foreach}
                        ...
                        <li class="dd-item dd3-item mb-1" id="new_variant">
                            ...
                            <div class="flex flex-wrap content-start bg-slate-100 pl-16 pr-2.5 py-2.5 min-h-[54px] rounded border bg-slate-100">
                                ...
                                <div class="grid gap-2 mt-2 grid-cols-7 w-full hidden" id="images-container-1">
                                    {foreach from=$images key=key item=image}
                                        <div class="flex flex-col items-center p-2 rounded border bg-slate-100">
                                            <input type="checkbox" id="image-list-{$image.id_item}" name="new_variants[images][]" value="{$image.id_item}" class="hidden"/>
                                            <label for="image-list-{$image.id_item}">
                                                <img class="object-cover w-full rounded aspect-square" src="{$image.filename}" alt="">
                                            </label>
                                        </div>
                                    {/foreach}
                                ...
                                <div class="grid gap-2 mt-2 grid-cols-1 w-full hidden" id="files-container-1">
                                    {foreach from=$files key=key item=file}
                                        <div class="flex flex-col items-center p-2 rounded border bg-slate-100">
                                            <input type="checkbox" id="file-list-{$file.id_item}" name="new_variants[files][]" value="{$file.id_item}" class="hidden"/>
                                            <label for="file-list-{$file.id_item}">
                                                <div class="flex items-center h-6">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-1.5 text-slate-500">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m18.375 12.739-7.693 7.693a4.5 4.5 0 0 1-6.364-6.364l10.94-10.94A3 3 0 1 1 19.5 7.372L8.552 18.32m.009-.01-.01.01m5.699-9.941-7.81 7.81a1.5 1.5 0 0 0 2.112 2.13"></path>
                                                    </svg>
                ...
                <button type="button" id="addVariant" data-variants-count="{$variants|count+1}" class="px-6 py-2 text-xs text-center text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none focus:ring-blue-300">
            ...
            <div class="w-full">
                ...
                <textarea name="annotation" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Краткое описание">{if isset($view.annotation)}{$view.annotation}{/if}</textarea>
            </div>
            <div class="w-full mt-5">
                <label class="block text-sm text-gray-900 mb-2">Подробное описание</label>
                <textarea name="text" id="text" class="editor">{if isset($view.text)}{$view.text}{/if}</textarea>
            </div>
        </div>
        ...
        <div class="w-3/12 bg-white shadow rounded p-4">
            ...
            {if isset($view.id_item)}
            <div class="flex-wrap p-4 bg-slate-50 rounded border border-dashed border-gray-300 flex mb-2 *:text-xs hidden" id="list" data-page-id="{$view.id_item}" data-module-id="shop">
            ...
            {if $files|count > 0}
                {foreach from=$files key=key item=item}
                    <input type='hidden' name='current_files[]' value='{$item.id_item}'>
                {/foreach}
            <div class="w-full mb-5">
                ...
                {foreach from=$files key=key item=item}
                    <div class="li-file p-3 mt-2 bg-slate-100 rounded-md flex flex-wrap content-center">
                        <div class="flex items-center h-6 w-4/5">
                            ...
                            <span class="editSpan font-medium text-slate-500 text-xs">{if empty($item.description)}{$item.filename|substr:20}{else}{$item.description}{/if}</span>
                            <input class="editInput hidden w-full appearance-none bg-white text-gray-700 border border-gray-400 rounded py-1 px-1
					focus:outline-none focus:border-gray-500 text-xs" type='text' name='file_name[{$item.id_item}]' value='{if empty($item.description)}{$item.filename|substr:20}{else}{$item.description}{/if}'>
                        </div>
                        <div class="ml-auto self-center h-6">
                            <input type='hidden' name='files[]' value='{$item.id_item}'>
                            ...
                            <button type="button" class="save_name_file hidden bg-slate-200 hover:bg-slate-700 text-slate-500 hover:text-white w-6 h-6 self-center rounded" data-id-file="{$item.id_item}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3 mx-1.5 my-1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                                </svg>
                            </button>
                            <button type="button" class="cancel hidden bg-slate-200 hover:bg-slate-700 text-slate-500 hover:text-white w-6 h-6 self-center rounded" data-id-file="{$item.id_item}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3 mx-1.5 my-1.5">
            ...
            {if $images|count > 0}
                {foreach from=$images key=key item=item}
                    <input type='hidden' name='current_images[]' value='{$item.id_item}'>
                {/foreach}
            <div class="w-full mb-5">
                ...
                <div class="grid grid-cols-1 gap-2 mt-2 grid-cols-4">
                    ...
                    <div class="li-image flex flex-col items-center p-2 border rounded bg-slate-100">
                        <img class="object-cover w-full rounded aspect-square" src="{$item.filename}" alt="">
                        <div class="flex mt-2">
                            <input type='hidden' name='images[]' value='{$item.id_item}'>
